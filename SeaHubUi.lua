...
-- 🌈 Hiệu ứng cầu vồng động
task.spawn(function()
    local RunService = game:GetService("RunService")
    while task.wait() do
        for i = 0, 255 do
            local color = Color3.fromHSV(i / 255, 1, 1)
            -- Cập nhật các màu chính
            getgenv().UIColor["Button Color"] = color
            getgenv().UIColor["Section Text Color"] = color
            getgenv().UIColor["Title Text Color"] = color
            getgenv().UIColor["Page Selected Color"] = color
            getgenv().UIColor["Toggle Border Color"] = color
            getgenv().UIColor["Section Underline Color"] = color

            -- Nếu GUI đã được tạo, cập nhật luôn
            local gui = game.CoreGui:FindFirstChild("Crystal Hub GUI")
            if gui then
                for _, obj in ipairs(gui:GetDescendants()) do
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                        obj.TextColor3 = getgenv().UIColor["Text Color"]
                    elseif obj:IsA("Frame") or obj:IsA("ImageLabel") then
                        if obj.Name == "ButtonBG" or obj.Name == "Linesec" then
