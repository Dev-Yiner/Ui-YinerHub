...
local T1UIColor = {
	...
	["Logo Image"] = "http://www.roblox.com/asset/?id=130128964253063",
...
-- 🌈 Hiệu ứng cầu vồng toàn bộ giao diện (ổn định, không lỗi)
task.spawn(function()
    local RunService = game:GetService("RunService")
    while true do
        for i = 0, 255 do
            local color = Color3.fromHSV(i / 255, 1, 1)
            local textColor = Color3.fromHSV((i / 255 + 0.5) % 1, 0.3, 1) -- màu chữ dịu hơn để dễ đọc

            -- cập nhật toàn bộ bảng màu
            for key, _ in pairs(getgenv().UIColor) do
                if string.find(string.lower(key), "color") then
                    getgenv().UIColor[key] = color
                end
            end
            getgenv().UIColor["Text Color"] = textColor
            getgenv().UIColor["GUI Text Color"] = textColor
            getgenv().UIColor["Toggle Desc Color"] = textColor

            -- cập nhật trực tiếp UI
            local gui = game.CoreGui:FindFirstChild("Crystal Hub GUI")
            if gui then
                for _, obj in ipairs(gui:GetDescendants()) do
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                        obj.TextColor3 = textColor
                    elseif obj:IsA("Frame") or obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
                        -- loại trừ vài phần không nên đổi hoàn toàn (vd: scrollbar)
                        if not string.find(string.lower(obj.Name), "scroll") then
